...
def init_database():
    ...
    new_columns = [
        ...
        ("ortalama_tedarik_suresi", "REAL DEFAULT 0"),
        ("ortalama_ek_sure", "REAL DEFAULT 0"),
        ("depo_stok", "REAL DEFAULT 0"),
        ("min_sevk_miktari", "INTEGER DEFAULT 1"),
        ("min_siparis_miktari", "INTEGER DEFAULT 1"),
        ("guvenlik_stok", "REAL DEFAULT 0")
    ]
    ...
    # Magaza tablosuna yol suresi ekle
    magaza_new_columns = [
        ("yol_suresi", "INTEGER DEFAULT 1"),
        ("oncelik", "INTEGER DEFAULT 1")
    ]
    for col_name, col_type in magaza_new_columns:
        try:
            cursor.execute(f"ALTER TABLE magaza ADD COLUMN {col_name} {col_type}")
        except:
            pass
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            try:
                ...
                depo_stok = float(row_dict.get("depo_stok", 0) or 0)
                min_sevk_miktari = int(float(row_dict.get("min_sevk_miktari", 1) or 1))
                min_siparis_miktari = int(float(row_dict.get("min_siparis_miktari", 1) or 1))
                guvenlik_stok = float(row_dict.get("guvenlik_stok", 0) or 0)
            ...
            if existing:
                ...
                update_malzeme(
                    ...
                    depo_stok=depo_stok,
                    min_sevk_miktari=min_sevk_miktari,
                    min_siparis_miktari=min_siparis_miktari,
                    guvenlik_stok=guvenlik_stok
                )
...
def get_malzemeler(aktif_only: bool = True) -> pd.DataFrame:
    ...
    query = """
        SELECT
            ...
            COALESCE(min_sevk_miktari, 1) as min_sevk_miktari,
            COALESCE(min_siparis_miktari, 1) as min_siparis_miktari,
            COALESCE(guvenlik_stok, 0) as guvenlik_stok,
            aciklama,
            aktif,
            created_at,
            updated_at
...
def add_malzeme_simple(
    ...
    depo_stok: float = 0,
    min_sevk_miktari: int = 1,
    min_siparis_miktari: int = 1,
    guvenlik_stok: float = 0,
    aciklama: str = ""
) -> int:
    ...
    cursor.execute("""
        INSERT INTO malzeme (
            ...
            depo_stok, min_sevk_miktari, min_siparis_miktari, guvenlik_stok, aciklama
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
def get_magazalar(aktif_only: bool = True) -> pd.DataFrame:
    ...
    query = """
        SELECT
            ...
            kapasite_adet,
            m2,
            COALESCE(yol_suresi, 1) as yol_suresi,
            COALESCE(oncelik, 1) as oncelik,
            aktif,
            created_at,
            updated_at
        FROM magaza
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            try:
                ...
                yol_suresi = int(float(row_dict.get("yol_suresi", 1) or 1))
                oncelik = int(float(row_dict.get("oncelik", 1) or 1))
            except:
                kapasite_adet = 0
                m2 = 0
                yol_suresi = 1
